...
const UnderReservations = () => {
  ...
  const translations = {
    ...
    ar: {
      underReservations: 'الحجوزات الحالية',
  ...
  const downloadExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, translations[language].underReservations);
    XLSX.writeFile(workbook, 'Under_Reservations.xlsx');
  };

  return (
    <div>
      <h1>{translations[language].underReservations}</h1>
